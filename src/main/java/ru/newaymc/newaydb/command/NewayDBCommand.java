...
public class NewayDBCommand {
    ...
    public static void registerCommand(RegisterCommandsEvent event) {
        if (event.getCommandSelection() == Commands.CommandSelection.INTEGRATED)
            event.getDispatcher()
                    .register(Commands.literal("newaydb").requires(s -> s.hasPermission(3)).then(Commands.literal("playerdata").then(Commands.literal("save").then(Commands.argument("force", StringArgumentType.word()).executes(arguments -> {
